...
struct EngineCross::Opcode EngineCross::codes[] = {
  // 00
  {_brk, _impl},
  {_ora, _xind},
  {_undoc, _illegal},
...
void EngineCross::usage() {
  printf("CPU == 'CROSS'\n");
  printf("  --label-file=<label file>\n");
  printf("\n");
}

void EngineCross::initialise() {
...
int EngineCross::disassemble(long addr, OutputItem *out) {
  int instruction;
  EngineCross::Opcode *opcode;
  int immVal;
  int inst;
  char label[MAXSTR];
  long target;
